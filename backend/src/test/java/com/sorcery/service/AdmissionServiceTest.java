...
class AdmissionServiceTest {
        ...
        // Test for the case where multiple candidates tie at the cutoff score
        @Test
        void tieCandidatesWithLastAcceptedAlsoAccepted() {
                Application c1 = buildApp("A101", "Rita", "Uno", 13, "cunning", "gossip", "2026-02-01"); // 17 puntos
                Application c2 = buildApp("A102", "Rita", "Dos", 13, "wit", "gossip", "2026-02-01"); // 15 puntos
                Application c3 = buildApp("A103", "Rita", "Tres", 14, "courage", "gossip", "2026-02-01"); // 10 puntos
                Application c4 = buildApp("A104", "Rita", "Cuatro", 15, "loyalty", "gossip", "2026-02-01"); // 10 puntos
                Application c5 = buildApp("A105", "Rita", "Cinco", 16, "wit", "gossip", "2026-02-01"); // 10 puntos
                rules.setPlaces(3);

                List<AdmissionResult> results = admissionService.processAdmissions(
                                List.of(c1, c2, c3, c4, c5), rules);
                long acceptedCount = results.stream()
                                .filter(r -> "ACCEPTED".equals(r.getStatus()))
                                .count();
                assertEquals(5, acceptedCount, "All three tied candidates at the cutoff score should be accepted");
        }
